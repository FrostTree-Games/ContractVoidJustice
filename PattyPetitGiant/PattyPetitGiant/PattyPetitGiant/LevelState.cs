            ...
            for (int i = 0; i < rooms.GetLength(0); i++)
            {
                for (int j = 0; j < rooms.GetLength(1); j++)
                {
                    ...
                    else if (rooms[i, j].attributes.Contains("start"))
                    {
                        ...
                        entityList.Add(new MolotovEnemy(this, new Vector2((currentRoomX + 8) * GlobalGameConstants.TileSize.X, (currentRoomY + 8) * GlobalGameConstants.TileSize.Y) + (GlobalGameConstants.TileSize * 3)));
                    }
